...
__global__ void output_agent_agent_to_VBO(xmachine_memory_agent_list* agents, float4* vbo, float3 centralise, int population_width){
...
void initVisualisation()
{
	...
	// create TBO
	createTBO( &agent_default_tbo, &agent_default_displacementTex, xmachine_memory_agent_MAX * sizeof( float4));
...
void runCuda()
{
	...
	if (get_agent_agent_default_count() > 0)
	{
		...
		gpuErrchk(cudaGLMapBufferObject( (void**)&dptr, agent_default_tbo));
		//cuda block size
		...
		output_agent_agent_to_VBO<<< grid, threads>>>(get_device_agent_default_agents(), dptr, centralise, population_width);
		gpuErrchkLaunch();
		// unmap buffer object
		gpuErrchk(cudaGLUnmapBufferObject(agent_default_tbo));
	}
