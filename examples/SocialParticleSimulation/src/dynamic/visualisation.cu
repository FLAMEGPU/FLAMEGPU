...
cudaGraphicsResource_t agent_state1_cgr;
GLuint agent_state1_tbo;
GLuint agent_state1_displacementTex;
...
__global__ void output_agent_agent_to_VBO(xmachine_memory_agent_list* agents, glm::vec4* vbo, glm::vec3 centralise){
    ...
    vbo[index].w = agents->strategy[index];
}
...
void initVisualisation()
{
	...
	// create TBO
	createTBO(&agent_state1_cgr, &agent_state1_tbo, &agent_state1_displacementTex, xmachine_memory_agent_MAX * sizeof( glm::vec4));
...
void runCuda()
{
	...
	if (get_agent_agent_state1_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
        size_t accessibleBufferSize = 0;
        gpuErrchk(cudaGraphicsMapResources(1, &agent_state1_cgr));
		gpuErrchk(cudaGraphicsResourceGetMappedPointer( (void**)&dptr, &accessibleBufferSize, agent_state1_cgr));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_agent_state1_count()/threads_per_tile);
		...
		output_agent_agent_to_VBO<<< grid, threads>>>(get_device_agent_state1_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
        gpuErrchk(cudaGraphicsUnmapResources(1, &agent_state1_cgr));
	}
...
void display()
{
	...
	//Draw agent Agents in state1 state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, agent_state1_displacementTex);
	//loop
	for (int i=0; i< get_agent_agent_state1_count(); i++){
...
void keyboard( unsigned char key, int /*x*/, int /*y*/)
{
	...
	case(27) :
		...
		deleteTBO( &agent_state1_cgr, &agent_state1_tbo);
