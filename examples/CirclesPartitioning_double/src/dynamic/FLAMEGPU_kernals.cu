...
__constant__ int d_xmachine_memory_Circle_count;

/* Agent state count constants */

__constant__ int d_xmachine_memory_Circle_default_count;
...
__constant__ int d_tex_xmachine_message_location_id_offset;texture<int2, 1, cudaReadModeElementType> tex_xmachine_message_location_x;
__constant__ int d_tex_xmachine_message_location_x_offset;texture<int2, 1, cudaReadModeElementType> tex_xmachine_message_location_y;
__constant__ int d_tex_xmachine_message_location_y_offset;texture<int2, 1, cudaReadModeElementType> tex_xmachine_message_location_z;
__constant__ int d_tex_xmachine_message_location_z_offset;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Dyanamically created Circle agent functions */

/** reset_Circle_scan_input
 * Circle agent reset scan input function
 * @param agents The xmachine_memory_Circle_list agent list
 */
__global__ void reset_Circle_scan_input(xmachine_memory_Circle_list* agents){
...
/** scatter_Circle_Agents
 * Circle scatter agents function (used after agent birth/death)
 * @param agents_dst xmachine_memory_Circle_list agent list destination
 * @param agents_src xmachine_memory_Circle_list agent list source
 * @param dst_agent_count index to start scattering agents from
 */
__global__ void scatter_Circle_Agents(xmachine_memory_Circle_list* agents_dst, xmachine_memory_Circle_list* agents_src, int dst_agent_count, int number_to_scatter){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
...
/** append_Circle_Agents
 * Circle scatter agents function (used after agent birth/death)
 * @param agents_dst xmachine_memory_Circle_list agent list destination
 * @param agents_src xmachine_memory_Circle_list agent list source
 * @param dst_agent_count index to start scattering agents from
 */
__global__ void append_Circle_Agents(xmachine_memory_Circle_list* agents_dst, xmachine_memory_Circle_list* agents_src, int dst_agent_count, int number_to_append){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
...
/** add_Circle_agent
 * Continuous Circle agent add agent function writes agent data to agent swap
 * @param agents xmachine_memory_Circle_list to add agents to 
 * @param id agent variable of type int
 * @param x agent variable of type double
 * @param y agent variable of type double
 * @param z agent variable of type double
 * @param fx agent variable of type double
 * @param fy agent variable of type double
 */
template <int AGENT_TYPE>
__device__ void add_Circle_agent(xmachine_memory_Circle_list* agents, int id, double x, double y, double z, double fx, double fy){
...
__device__ void add_Circle_agent(xmachine_memory_Circle_list* agents, int id, double x, double y, double z, double fx, double fy){
    add_Circle_agent<DISCRETE_2D>(agents, id, x, y, z, fx, fy);
}

/** reorder_Circle_agents
 * Continuous Circle agent areorder function used after key value pairs have been sorted
 ...
 */
__global__ void reorder_Circle_agents(unsigned int* values, xmachine_memory_Circle_list* unordered_agents, xmachine_memory_Circle_list* ordered_agents)
{
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
 ...
 * @param z agent variable of type double
 */
__device__ void add_location_message(xmachine_message_location_list* messages, int id, double x, double y, double z){
...
__device__ unsigned int message_location_hash(int3 gridPos)
{
	...
	/** hist_location_messages
		 ...
		 */
	__global__ void hist_location_messages(uint* local_bin_index, uint* unsorted_index, int* global_bin_count, xmachine_message_location_list* messages)
	{
	 ...
	 */
	 __global__ void reorder_location_messages(uint* local_bin_index, uint* unsorted_index, int* pbm_start_index, xmachine_message_location_list* unordered_messages, xmachine_message_location_list* ordered_messages)
	{
		int index = (blockIdx.x *blockDim.x) + threadIdx.x;
...
__device__ int load_next_location_message(xmachine_message_location_list* messages, xmachine_message_location_PBM* partition_matrix, int3 relative_cell, int cell_index_max, int3 agent_grid_cell, int cell_index)
{
	...
	//Using texture cache
  temp_message.id = tex1Dfetch(tex_xmachine_message_location_id, cell_index + d_tex_xmachine_message_location_id_offset); temp_message.x = tex1DfetchDouble(tex_xmachine_message_location_x, cell_index + d_tex_xmachine_message_location_x_offset);temp_message.y = tex1DfetchDouble(tex_xmachine_message_location_y, cell_index + d_tex_xmachine_message_location_y_offset);temp_message.z = tex1DfetchDouble(tex_xmachine_message_location_z, cell_index + d_tex_xmachine_message_location_z_offset);
 ...
 *
 */
__global__ void GPUFLAME_outputdata(xmachine_memory_Circle_list* agents, xmachine_message_location_list* location_messages){
	
	//continuous agent: index is agent position in 1D agent list
	int index = (blockIdx.x * blockDim.x) + threadIdx.x;
  
    //For agents not using non partitioned message input check the agent bounds
    if (index >= d_xmachine_memory_Circle_count)
        return;
    

	//SoA to AoS - xmachine_memory_outputdata Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_Circle agent;
 ...
 *
 */
__global__ void GPUFLAME_inputdata(xmachine_memory_Circle_list* agents, xmachine_message_location_list* location_messages, xmachine_message_location_PBM* partition_matrix){
 ...
 *
 */
__global__ void GPUFLAME_move(xmachine_memory_Circle_list* agents){
