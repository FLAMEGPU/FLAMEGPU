...
GLuint keratinocyte_default_tbo;
GLuint keratinocyte_default_displacementTex;

GLuint keratinocyte_resolve_tbo;
GLuint keratinocyte_resolve_displacementTex;
...
__global__ void output_keratinocyte_agent_to_VBO(xmachine_memory_keratinocyte_list* agents, float4* vbo, float3 centralise){
...
void initVisualisation()
{
	...
	// create TBO
	createTBO( &keratinocyte_default_tbo, &keratinocyte_default_displacementTex, xmachine_memory_keratinocyte_MAX * sizeof( float4));
	
	createTBO( &keratinocyte_resolve_tbo, &keratinocyte_resolve_displacementTex, xmachine_memory_keratinocyte_MAX * sizeof( float4));
...
void runCuda()
{
	...
	if (get_agent_keratinocyte_default_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
		gpuErrchk(cudaGLMapBufferObject( (void**)&dptr, keratinocyte_default_tbo));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_keratinocyte_default_count()/threads_per_tile);
		...
		output_keratinocyte_agent_to_VBO<<< grid, threads>>>(get_device_keratinocyte_default_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
		gpuErrchk(cudaGLUnmapBufferObject(keratinocyte_default_tbo));
	}
	
	if (get_agent_keratinocyte_resolve_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
		gpuErrchk(cudaGLMapBufferObject( (void**)&dptr, keratinocyte_resolve_tbo));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_keratinocyte_resolve_count()/threads_per_tile);
		grid = dim3(tile_size, 1, 1);
		threads = dim3(threads_per_tile, 1, 1);
		...
		output_keratinocyte_agent_to_VBO<<< grid, threads>>>(get_device_keratinocyte_resolve_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
		gpuErrchk(cudaGLUnmapBufferObject(keratinocyte_resolve_tbo));
	}
...
void display()
{
	...
	//Draw keratinocyte Agents in default state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, keratinocyte_default_displacementTex);
	//loop
	for (int i=0; i< get_agent_keratinocyte_default_count(); i++){
	...
	//Draw keratinocyte Agents in resolve state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, keratinocyte_resolve_displacementTex);
	//loop
	for (int i=0; i< get_agent_keratinocyte_resolve_count(); i++){
		glVertexAttrib1f(vs_mapIndex, (float)i);
...
void keyboard( unsigned char key, int /*x*/, int /*y*/)
{
	...
	case( 27) :
		...
		deleteTBO( &keratinocyte_default_tbo);
		
		deleteTBO( &keratinocyte_resolve_tbo);
