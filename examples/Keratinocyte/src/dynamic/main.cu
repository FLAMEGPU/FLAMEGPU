	...
	for (int i=0; i< itterations; i++)
	{
		...
		if (OUTPUT_TO_XML)
		{
			saveIterationData(outputpath, i+1, 
				//default state keratinocyte agents
				get_host_keratinocyte_default_agents(), get_device_keratinocyte_default_agents(), get_agent_keratinocyte_default_count(),
				//resolve state keratinocyte agents
				get_host_keratinocyte_resolve_agents(), get_device_keratinocyte_resolve_agents(), get_agent_keratinocyte_resolve_count());
