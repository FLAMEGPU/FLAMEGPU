...
float3* arrow_vertices;
float3* arrow_normals;
int3* arrow_faces;
...
{
	//create TBO
	createTBO(&nm_instances_tbo, &nm_instances_tex, get_agent_navmap_MAX_count()* sizeof(float4));
	...
	if (useLargeVBO)
	{
		...
		//create VBOs
		createVBO(&arrow_verts_vbo, GL_ARRAY_BUFFER, get_agent_navmap_MAX_count()*arrow_v_count*sizeof(float3));
		createVBO(&arrow_elems_vbo, GL_ELEMENT_ARRAY_BUFFER, get_agent_navmap_MAX_count()*arrow_f_count*sizeof(int3));
		...
		glBindBuffer(GL_ARRAY_BUFFER, arrow_verts_vbo);
		verts = (float3*)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
		i,v=0;
		...
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, arrow_elems_vbo);
		faces = (int3*)glMapBuffer(GL_ELEMENT_ARRAY_BUFFER, GL_WRITE_ONLY);
		i,f=0;
		for (i=0;i<get_agent_navmap_MAX_count();i++){
	...
	{
		//create VBOs
		createVBO(&arrow_verts_vbo, GL_ARRAY_BUFFER, arrow_v_count*sizeof(float3));
		createVBO(&arrow_elems_vbo, GL_ELEMENT_ARRAY_BUFFER, arrow_f_count*sizeof(int3));

		//bind VBOs
		glBindBuffer(GL_ARRAY_BUFFER, arrow_verts_vbo);
		glBufferData(GL_ARRAY_BUFFER, arrow_v_count*sizeof(float3), arrow_vertices, GL_DYNAMIC_DRAW);
		glBindBuffer( GL_ARRAY_BUFFER, 0);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, arrow_elems_vbo);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, arrow_f_count*sizeof(int3), arrow_faces, GL_DYNAMIC_DRAW);
		glBindBuffer( GL_ARRAY_BUFFER, 0);
	}
