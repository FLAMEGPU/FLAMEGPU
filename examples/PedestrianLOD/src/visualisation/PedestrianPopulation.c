...
float3* lod1l_vertices;
float3* lod1l_normals;
int3* lod1l_faces;
//right keyframe primative data
float3* lod1r_vertices;
float3* lod1r_normals;
int3* lod1r_faces;
//buffer objects
...
float3* lod2l_vertices;
float3* lod2l_normals;
int3* lod2l_faces;
//right keyframe primative data
float3* lod2r_vertices;
float3* lod2r_normals;
int3* lod2r_faces;
//buffer objects
...
int lod3_f_count;
//left keyframe primative data
float3* lod3l_vertices;
float3* lod3l_normals;
int3* lod3l_faces;
//right keyframe primative data
float3* lod3r_vertices;
float3* lod3r_normals;
int3* lod3r_faces;
//buffer objects
...
{	
	...
	//run CUDA
	generate_instances_and_LOD(&p_instances_data1_tbo, &p_instances_data2_tbo);
...
{
	//create TBO
	createTBO(&p_instances_data1_tbo, &p_instances_data1_tex, get_agent_agent_MAX_count()* sizeof(float4));
	createTBO(&p_instances_data2_tbo, &p_instances_data2_tex, get_agent_agent_MAX_count()* sizeof(float4));
	registerBO(&p_instances_data1_tbo);
	registerBO(&p_instances_data2_tbo);

	//create VBOs (LOD1, LOD2, LOD3)
	createVBO(&lod1l_verts_vbo, GL_ARRAY_BUFFER, lod1_v_count*sizeof(float3));
	createVBO(&lod1_elem_vbo, GL_ELEMENT_ARRAY_BUFFER, lod1_f_count*sizeof(int3));
	createVBO(&lod1r_verts_vbo, GL_ARRAY_BUFFER, lod1_v_count*sizeof(float3));
	createVBO(&lod2l_verts_vbo, GL_ARRAY_BUFFER, lod2_v_count*sizeof(float3));
	createVBO(&lod2_elem_vbo, GL_ELEMENT_ARRAY_BUFFER, lod2_f_count*sizeof(int3));
	createVBO(&lod2r_verts_vbo, GL_ARRAY_BUFFER, lod2_v_count*sizeof(float3));
	createVBO(&lod3l_verts_vbo, GL_ARRAY_BUFFER, lod3_v_count*sizeof(float3));
	createVBO(&lod3_elem_vbo, GL_ELEMENT_ARRAY_BUFFER, lod3_f_count*sizeof(int3));
	createVBO(&lod3r_verts_vbo, GL_ARRAY_BUFFER, lod3_v_count*sizeof(float3));

	//bind VBOs LOD1
	glBindBuffer(GL_ARRAY_BUFFER, lod1l_verts_vbo);
	glBufferData(GL_ARRAY_BUFFER, lod1_v_count*sizeof(float3), lod1l_vertices, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, lod1_elem_vbo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, lod1_f_count*sizeof(int3), lod1l_faces, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, lod1r_verts_vbo);
	glBufferData(GL_ARRAY_BUFFER, lod1_v_count*sizeof(float3), lod1r_vertices, GL_DYNAMIC_DRAW);

	//bind VBOs LOD2
	glBindBuffer(GL_ARRAY_BUFFER, lod2l_verts_vbo);
	glBufferData(GL_ARRAY_BUFFER, lod2_v_count*sizeof(float3), lod2l_vertices, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, lod2_elem_vbo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, lod2_f_count*sizeof(int3), lod2l_faces, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, lod2r_verts_vbo);
	glBufferData(GL_ARRAY_BUFFER, lod2_v_count*sizeof(float3), lod2r_vertices, GL_DYNAMIC_DRAW);

	//bind VBOs lod3
	glBindBuffer(GL_ARRAY_BUFFER, lod3l_verts_vbo);
	glBufferData(GL_ARRAY_BUFFER, lod3_v_count*sizeof(float3), lod3l_vertices, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, lod3_elem_vbo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, lod3_f_count*sizeof(int3), lod3l_faces, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, lod3r_verts_vbo);
	glBufferData(GL_ARRAY_BUFFER, lod3_v_count*sizeof(float3), lod3r_vertices, GL_DYNAMIC_DRAW);
}
