	...
	for (int i=0; i< itterations; i++)
	{
		...
		if (OUTPUT_TO_XML)
		{
			saveIterationData(outputpath, i+1, 
				//unengaged state Man agents
				get_host_Man_unengaged_agents(), get_device_Man_unengaged_agents(), get_agent_Man_unengaged_count(),
				//engaged state Man agents
				get_host_Man_engaged_agents(), get_device_Man_engaged_agents(), get_agent_Man_engaged_count(),
				//default state Woman agents
				get_host_Woman_default_agents(), get_device_Woman_default_agents(), get_agent_Woman_default_count());
