...
<xsl:if test="gpu:xmodel/gpu:environment/gpu:initFunctions/gpu:initFunction/xmml:xagentOutputs/gpu:xagentOutput">
	// Kernel launch parameters for any xagentOutput from functions.
	int blockSize;
	int minGridSize;
	int gridSize;
	unsigned int new_agent_count;
</xsl:if>
...
<xsl:when test="xmml:xagentOutputs/gpu:xagentOutput">
	// Init function generates agents
	// Allocate and initialise to 0 a struct for the agent
<xsl:for-each select="xmml:xagentOutputs/gpu:xagentOutput">    xmachine_memory_<xsl:value-of select="xmml:xagentName"/> * agent = (xmachine_memory_<xsl:value-of select="xmml:xagentName"/>*)malloc(sizeof(xmachine_memory_<xsl:value-of select="xmml:xagentName"/>));
<xsl:variable name="xagentname" select="xmml:xagentName"/>
...
<xsl:text>    </xsl:text><xsl:value-of select="gpu:name"/>(<xsl:for-each select="xmml:xagentOutputs/gpu:xagentOutput">h_<xsl:value-of select="xmml:xagentName"/>s_<xsl:value-of select="xmml:state"/>, &amp;new_agent_count, agent<xsl:if test="position()!=last()">,</xsl:if></xsl:for-each>);<xsl:text>
</xsl:text>
    // Free the agent struct used in the functions.c file <xsl:for-each select="xmml:xagentOutputs/gpu:xagentOutput"><xsl:variable name="xagentname" select="xmml:xagentName"/><xsl:for-each select="../../../../../xmml:xagents/gpu:xagent[xmml:name=$xagentname]/xmml:memory/gpu:variable"><xsl:if test="xmml:arrayLength">
    free(agent-&gt;<xsl:value-of select="xmml:name"/>);
</xsl:if></xsl:for-each>    free(agent);
</xsl:for-each>
	if(new_agent_count &gt; 0 <xsl:if test="position()!=last()">&amp;&amp;</xsl:if>){
<!-- For each xagent output present -->
<xsl:for-each select="xmml:xagentOutputs/gpu:xagentOutput"><xsl:variable name="xagent_output_name" select="xmml:xagentName"/>
	// Ensure there is room for these agents in the output state
	if (h_xmachine_memory_<xsl:value-of select="xmml:xagentName"/>_<xsl:value-of select="xmml:state"/>_count + new_agent_count &gt; xmachine_memory_<xsl:value-of select="xmml:xagentName"/>_MAX){
		printf("Error: Buffer size of <xsl:value-of select="xmml:xagentName"/> agents in state <xsl:value-of select="xmml:state"/> exceeds maximum number of <xsl:value-of select="xmml:xagentName"/> agents %u from step function <xsl:value-of select="gpu:name"/> \n", xmachine_memory_<xsl:value-of select="xmml:xagentName"/>_MAX);
		exit(0);
	}

	// For each agent variable, copy the bit of memory as appropriate.
	copy_partial_xmachine_memory_<xsl:value-of select="xmml:xagentName"/>(d_<xsl:value-of select="xmml:xagentName"/>s_new, h_<xsl:value-of select="xmml:xagentName"/>s_<xsl:value-of select="xmml:state"/>, new_agent_count);

	// Append the new device array agents into the relevant state
	cudaOccupancyMaxPotentialBlockSizeVariableSMem(&amp;minGridSize, &amp;blockSize, append_<xsl:value-of select="xmml:xagentName"/>_Agents, no_sm, new_agent_count);
	gridSize = (new_agent_count + blockSize - 1) / blockSize;
	append_<xsl:value-of select="xmml:xagentName"/>_Agents &lt;&lt;&lt;gridSize, blockSize, 0, stream1 &gt;&gt;&gt;(d_<xsl:value-of select="xmml:xagentName"/>s_<xsl:value-of select="xmml:state"/>, d_<xsl:value-of select="xmml:xagentName"/>s_new, h_xmachine_memory_<xsl:value-of select="xmml:xagentName"/>_<xsl:value-of select="xmml:state"/>_count, new_agent_count);
	gpuErrchkLaunch();
...
<xsl:for-each select="gpu:xmodel/xmml:xagents/gpu:xagent">
<!-- @todo - heuristically decide if it is better to move the full SoA in one copy, or only move part of the agent dependant upon count and the maximum pop size, and the per-agent data quantity. -->
<!-- @todo - Determine the best approach for agent variable arrays. For small counts and small array sizes multiple copies is likely best. Large arrays would invoke significant copy overheads reducing bandwidth.  -->
void copy_partial_xmachine_memory_<xsl:value-of select="xmml:name"/>(xmachine_memory_<xsl:value-of select="xmml:name"/>_list * d_dst, xmachine_memory_<xsl:value-of select="xmml:name"/>_list * h_src, unsigned int count){
	...
	<xsl:for-each select="xmml:memory/gpu:variable"><xsl:if test="xmml:arrayLength"> 
		<!-- Full copy of arrays may be better than partial copies, so this has been left in for now as an alternative.
		gpuErrchk(cudaMemcpy(d_dst-&gt;<xsl:value-of select="xmml:name"/>, h_src-&gt;<xsl:value-of select="xmml:name"/>, xmachine_memory_<xsl:value-of select="../../xmml:name" />_MAX * <xsl:if test="xmml:arrayLength"><xsl:value-of select="xmml:arrayLength"/> * </xsl:if>sizeof(<xsl:value-of select="xmml:type"/>), cudaMemcpyHostToDevice));
		-->
		for(unsigned int i = 0; i &lt; <xsl:value-of select="xmml:arrayLength"/>; i++){
...
<xsl:if test="gpu:xmodel/gpu:environment/gpu:stepFunctions/gpu:stepFunction/xmml:xagentOutputs/gpu:xagentOutput">
// Kernel launch parameters for any xagentOutput from functions.
int blockSize;
int minGridSize;
int gridSize;
...
<xsl:choose>
<xsl:when test="xmml:xagentOutputs/gpu:xagentOutput">
// Step function generates agents
// Allocate and initialise to 0 a struct for the agent
<xsl:for-each select="xmml:xagentOutputs/gpu:xagentOutput">    xmachine_memory_<xsl:value-of select="xmml:xagentName"/> * agent = (xmachine_memory_<xsl:value-of select="xmml:xagentName"/>*)malloc(sizeof(xmachine_memory_<xsl:value-of select="xmml:xagentName"/>));
<xsl:variable name="xagentname" select="xmml:xagentName"/>
<xsl:for-each select="../../../../../xmml:xagents/gpu:xagent[xmml:name=$xagentname]/xmml:memory/gpu:variable">
...
<xsl:for-each select="xmml:memory/gpu:variable">
<xsl:if test="xmml:arrayLength">  for (int i=0;i&lt;<xsl:value-of select="xmml:arrayLength"/>;i++){
    h_<xsl:value-of select="../../xmml:name"/>s-&gt;<xsl:value-of select="xmml:name"/>[(i * xmachine_memory_<xsl:value-of select="../../xmml:name"/>_MAX) + index] = agent-&gt;<xsl:value-of select="xmml:name"/>[i];
  }
</xsl:if>
<xsl:if test="not(xmml:arrayLength)">  h_<xsl:value-of select="../../xmml:name"/>s-&gt;<xsl:value-of select="xmml:name"/>[index] = agent-&gt;<xsl:value-of select="xmml:name"/>;
</xsl:if>
</xsl:for-each>
}
</xsl:for-each>
